...
class RLAgent:
    def __init__(self, state_dim, action_dim, max_action, gamma=0.99, tau=0.005, device='cpu', buffer_size=100000, batch_size=8):
        self.gamma = gamma
        self.tau = tau
    ...
    def choose_action(self, state, noise=0.1):
        ...
        #print(f'Action generated by policy net: {action}')
        # Add some noise for exploration
        action = action + np.random.normal(0, noise, size=action.shape)
        clipped_action = np.clip(action, -self.max_action, self.max_action)
        #print(f'Action after noising and clipping: {action}')
